    ...
    <body>
        ...
        <table class="meniu" style = "margin-top:10px; margin-bottom:10px;">
			<tr>
				...
				<td> <a class="button" href="interogari10.php"> >> </td>
			</tr>
		</table>
        <div class="enunt">
            9. Meciurile in care antrenorii ambelor echipe au varsta de peste 50 de ani:
        </div>
        <?php
            $qp11 = "SELECT DENUMIRETARA AS DENUMIRE1 FROM MECIURI JOIN ECHIPE ON ECHIPE.ECHIPAID = MECIURI.ECHIPA1ID JOIN STADIOANE ON MECIURI.STADIONID = STADIOANE.STADIONID WHERE ECHIPA1ID IN (SELECT ECHIPAID FROM ECHIPE JOIN ANTRENORI ON ECHIPE.ANTRENORID = ANTRENORI.ANTRENORID WHERE VARSTA > 50) AND ECHIPA2ID IN (SELECT ECHIPAID FROM ECHIPE JOIN ANTRENORI ON ECHIPE.ANTRENORID = ANTRENORI.ANTRENORID WHERE VARSTA > 50) ORDER BY MECIID ASC";
            ...
            $query1 = "SELECT ECHIPA1ID, ECHIPA2ID, DENUMIRE, ECHIPA1SCOR, ECHIPA2SCOR, SCOR1PEN, SCOR2PEN, DATAMECI, TIPMECI
                        ...
                        WHERE (ECHIPA1ID IN (SELECT ECHIPAID FROM ECHIPE JOIN ANTRENORI ON ECHIPE.ANTRENORID = ANTRENORI.ANTRENORID WHERE VARSTA > 50)
                        AND ECHIPA2ID IN (SELECT ECHIPAID FROM ECHIPE JOIN ANTRENORI ON ECHIPE.ANTRENORID = ANTRENORI.ANTRENORID WHERE VARSTA > 50)) ORDER BY MECIID ASC";
